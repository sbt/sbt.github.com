...
datatype is a code generation library and an sbt autoplugin that generates
...
```scala
addSbtPlugin("org.scala-sbt" % "datatype-plugin" % "0.0.2")
```

Your datatype definitions should be placed by default in
`sourceDirectory.value / "datatype"`. This location can be changed by setting
a new location in your build definition:
...
to indicate a the version `since` the field exists and a `default` value in the
datatype definition:
